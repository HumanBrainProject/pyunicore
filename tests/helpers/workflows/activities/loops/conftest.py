...
def loop_body() -> loops.Body:
    activities = [_activities.ModifyVariable(id="test-modify-variable-id", variable_name="test-variable", expression="test-expression")]
    transitions = [transition.Transition(from_="here", to="there", condition="test-condition")]
    return loops.Body(activities=activities, transitions=transitions, condition="test-body-condition")
...
def expected_loop_body() -> Dict:
    return {
        "activities": [
            {"id": "test-modify-variable-id", "type": "ModifyVariable", "variableName": "test-variable", "expression": "test-expression"},
            ],
        "transitions": [
